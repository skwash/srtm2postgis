...
import database_cas_test 

import read_data_cas
...
class TestDatabase(unittest.TestCase):
  def setUp(self):
    # Drop all tables
    self.db_cas = read_data_cas.ColumnFamilyCass(database_cas_test.keyspace, database_cas_test.cf_name, database_cas_test.nodelist) 
    self.db_cas.purge()
  ...
  def testDatabaseEmpty(self):
    # This isn't really possible in Cassandra.
    self.assert_(True)
  
  def testTableAltitudeExists(self):
    # We probably can't need to do this.
   self.assert_(True)
